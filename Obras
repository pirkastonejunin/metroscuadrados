...
router.post('/:id/tareas', authAdmin, async (req, res) => {
  try {
    const id = toObjectId(req.params.id);
    if (!id) return res.status(400).json({ error: 'id inválido' });
    const { tipoTrabajo, m2Presupuestados, fechaFinEstimada, notas } = req.body || {};
...
router.put('/:obraId/tareas/:tareaId', authAdmin, async (req, res) => {
  try {
    ...
    const resultado = await conReintento(async () => {
      ...
      const { colocadorId, m2Presupuestados, fechaFinEstimada, notas, estado } = req.body || {};
