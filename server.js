...
async function guardarRendimientoCache(storeId, productId, tipo, cobertura, handle, envase, asociados) {
  const intentar = async () => {
    const col = await getRendimientosCollection();
    const datos = { store_id: storeId, product_id: parseInt(productId, 10), tipo, cobertura };
    if (handle) datos.handle = String(handle).toLowerCase();
    if (envase) datos.envase = envase;
    if (asociados !== undefined) datos.asociados = asociados || '';
    await col.updateOne(
...
async function fetchAllProducts(storeId, accessToken) {
  ...
  while (true) {
    const response = await fetch(
      API_BASE + '/' + storeId + '/products?per_page=200&page=' + page + '&fields=id,name,variants,handle,categories',
      { headers: apiHeaders(accessToken) }
    );
      ...
      return {
        ...
        categoria: (p.categories && p.categories.length)
          ? p.categories.map(function (c) {
              return c.name && (c.name.es || Object.values(c.name)[0]);
            }).filter(Boolean).join(' / ')
          : '',
        cobertura: cache ? { value: cache.cobertura } : null,
        tipo: cache ? { value: cache.tipo } : null,
        envase: cache && cache.envase ? { value: cache.envase } : null,
        asociados: cache && cache.asociados ? { value: cache.asociados } : null
      };
    });
...
app.post('/api/bulto-cache/:productId', async (req, res) => {
  try {
    ...
    const { cobertura, tipo, handle, envase, asociados } = req.body;
    ...
    await guardarRendimientoCache(parseInt(storeId, 10), productId, tipo, coberturaFinal, handle, envase, asociados);
    res.json({ ok: true });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
app.post('/api/bulto/:productId', async (req, res) => {
  try {
    ...
    await guardarRendimientoCache(store_id, productId, tipo, coberturaFinal, handle, envase, asociados);
...
app.get('/public/catalogo-precios', async (req, res) => {
  ...
  try {
    ...
    const productos = rendimientos
      ...
      .map((r) => ({ handle: String(r.handle).toLowerCase(), coberturaCaja: parseFloat(r.cobertura), tipoUnidad: r.tipo, envase: r.envase || 'caja' }));
...
app.get('/public/bulto-handle/:handle', async (req, res) => {
  ...
  try {
    ...
    const handleBuscado = String(req.params.handle).toLowerCase();
    const intentar = async () => {
      const col = await getRendimientosCollection();
      // Busca por handle normalizado; el fallback con regex cubre
      // registros viejos guardados antes de normalizar a minusculas.
      let doc = await col.findOne({ store_id: store.store_id, handle: handleBuscado });
      if (!doc) {
        doc = await col.findOne({
          store_id: store.store_id,
          handle: { $regex: '^' + handleBuscado.replace(/[.*+?^${}()|[\]\\]/g, '\\$&') + '$', $options: 'i' }
        });
      }
      return doc;
    };
    ...
    res.json({
      ...
      envase: cache && cache.envase ? cache.envase : 'caja',
      asociados: asociados
    });
  } catch (err) {
    res.status(500).json({ coberturaCaja: null, tipoUnidad: null, envase: 'caja', asociados: [], error: err.message });
  }
});

// Convierte el texto "SKU:rendimiento|SKU:rendimiento" en una lista de
// productos reales, buscando cada SKU en la API de Tiendanube.
// Cachea el resultado por SKU en Mongo para no repetir llamadas.
async function resolverAsociados(store, textoAsociados) {
  const partes = String(textoAsociados).split('|').map((s) => s.trim()).filter(Boolean);
  const resultado = [];

  for (const parte of partes) {
    const idx = parte.lastIndexOf(':');
    if (idx === -1) continue;
    const sku = parte.slice(0, idx).trim();
    const rinde = parseFloat(parte.slice(idx + 1).trim());
    if (!sku || !rinde || rinde <= 0) continue;

    try {
      const info = await buscarProductoPorSku(store, sku);
      if (info) {
        resultado.push({
          sku: sku,
          rinde: rinde,
      ...
      // Si falla la busqueda de un SKU, seguimos con los demas
    }
  }
...
async function buscarProductoPorSku(store, sku) {
  const col = await getRendimientosCollection();
  const cacheKey = 'skucache_' + store.store_id + '_' + sku.toLowerCase();
  ...
  for (const p of productos) {
    ...
    for (const v of p.variants) {
      if (v.sku && String(v.sku).toLowerCase() === sku.toLowerCase()) {
        const handle = p.handle && (p.handle.es || Object.values(p.handle)[0]);
        const info = {
          nombre: p.name && (p.name.es || Object.values(p.name)[0]),
          precio: v.price ? parseFloat(v.price) : null,
          url: handle ? '/productos/' + handle + '/' : null,
          variant_id: v.id,
          imagen: p.images && p.images[0] ? p.images[0].src : null
        };
        await col.updateOne({ _id: cacheKey }, { $set: { info } }, { upsert: true });
        return info;
      }
    }
  }
