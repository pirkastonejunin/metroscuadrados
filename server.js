...
const KEY_ATRIBUTOS = 'atributos_extra';
...
app.get('/api/products', async (req, res) => {
  try {
    ...
    const response = await fetch(
      API_BASE + '/' + store_id + '/products?per_page=200&fields=id,name,variants,handle',
      { headers: apiHeaders(access_token) }
    );
    ...
    const withCobertura = await Promise.all(
      products.map(async (p) => {
        ...
        const atributos = await getMetafieldValue(store_id, access_token, p.id, KEY_ATRIBUTOS);
        return {
          id: p.id,
...
// Guarda / actualiza cuanto rinde la caja de un producto y en que unidad
app.post('/api/bulto/:productId', async (req, res) => {
  try {
    ...
    var tiposValidos = ['m2', 'ml', 'litro', 'unidad'];
...
// Lista de materiales/texturas disponibles (propia, guardada en Mongo,
// no en Tiendanube). Se usa para llenar el desplegable en /admin.html.
app.get('/api/etiquetas', async (req, res) => {
  try {
    const db = await readDb();
    res.json(db.etiquetas || []);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
});
...
app.post('/api/etiquetas', async (req, res) => {
  try {
    ...
    if (!valor || !valor.trim()) {
      return res.status(400).json({ error: 'Valor vacio' });
    }
    const db = await readDb();
    const etiquetas = db.etiquetas || [];
    if (etiquetas.indexOf(valor.trim()) === -1) {
      etiquetas.push(valor.trim());
    }
    await writeDb({ etiquetas });
    res.json(etiquetas);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
app.delete('/api/etiquetas/:valor', async (req, res) => {
  try {
    const db = await readDb();
    const etiquetas = (db.etiquetas || []).filter((e) => e !== req.params.valor);
    await writeDb({ etiquetas });
    res.json(etiquetas);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
// Guarda / actualiza los atributos de clasificacion de un producto
// (texto tipo "Textura:Madera|Material:Cemento")
app.post('/api/atributos/:productId', async (req, res) => {
  try {
    const { store_id, access_token } = await getStoreCreds();
    const productId = req.params.productId;
    const { atributos } = req.body;

    await saveMetafieldValue(store_id, access_token, productId, KEY_ATRIBUTOS, atributos || '');
...
// Endpoint PUBLICO: catalogo completo para el buscador por material.
// Devuelve nombre, foto, precio, material, y la URL real del producto.
app.get('/public/catalogo', async (req, res) => {
  res.set('Access-Control-Allow-Origin', '*');
  try {
    const { store_id, access_token } = await getStoreCreds();

    const storeResponse = await fetch(API_BASE + '/' + store_id + '/store', {
      headers: apiHeaders(access_token)
    });
    ...
    const response = await fetch(
      API_BASE + '/' + store_id + '/products?per_page=200&fields=id,name,handle,variants,images',
      { headers: apiHeaders(access_token) }
    );
    const products = await response.json();
    ...
    const catalogo = await Promise.all(
      products.map(async (p) => {
        const atributos = await getMetafieldValue(store_id, access_token, p.id, KEY_ATRIBUTOS);
        const nombre = p.name && (p.name.es || Object.values(p.name)[0]);
        const handle = p.handle && (p.handle.es || Object.values(p.handle)[0]);
        const precio = p.variants && p.variants[0] ? p.variants[0].price : null;
        const imagen = p.images && p.images[0] ? p.images[0].src : null;
        return {
          id: p.id,
          nombre,
          precio,
          imagen,
          url: storeUrl && handle ? storeUrl + '/productos/' + handle + '/' : null,
          material: atributos ? atributos.value : ''
        };
      })
    );

    res.json({ productos: catalogo.filter((p) => p.url) });
  } catch (err) {
...
app.get('/public/bulto/:productId', async (req, res) => {
  ...
  try {
    ...
    const tipo = await getMetafieldValue(store_id, access_token, req.params.productId, KEY_TIPO);
    res.json({
