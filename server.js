...
async function guardarRendimientoCache(storeId, productId, tipo, cobertura, handle, envase) {
  const intentar = async () => {
...
app.post('/api/bulto-cache/:productId', async (req, res) => {
  try {
    ...
    const { cobertura, tipo, handle, envase } = req.body;
    ...
    await guardarRendimientoCache(parseInt(storeId, 10), productId, tipo, coberturaFinal, handle, envase);
    res.json({ ok: true });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
app.post('/api/bulto/:productId', async (req, res) => {
  try {
    ...
    await guardarRendimientoCache(store_id, productId, tipo, coberturaFinal, handle, envase);
...
app.get('/public/bulto-handle/:handle', async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ coberturaCaja: null, tipoUnidad: null, envase: 'caja', error: err.message });
  }
});
